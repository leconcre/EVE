# eve_gui.py - EVE v3.0 COM GROQ INTEGRADO
"""
...
# Carrega variáveis de ambiente (.env) — a chave da API mora lá, não no código
...
# Chave vem do arquivo .env (GROQ_API_KEY=...) — NUNCA hardcoded no código
...
class EVEApp(ctk.CTk):
    def __init__(self):
        ...
        # - request_counter identifica cada envio
        # - active_request é o pedido cuja resposta ainda interessa
        # - eve_lock impede duas threads de usarem a EVE ao mesmo tempo
        #   (o motor não é thread-safe)
        self.request_counter = 0
        self.active_request = None
        self.eve_lock = threading.Lock()
    ...
    def _init_engines(self):
        ...
        if GROQ_AVAILABLE and GROQ_API_KEY:
            ...
            print("⚠️ GROQ_API_KEY não definida no .env - modo código desabilitado")
    ...
    # ═══════════════════════════════════════════════════════════════════
    # BOLHA DE STREAMING (preview enquanto o modelo local gera)
    # ═══════════════════════════════════════════════════════════════════

    def _start_stream_bubble(self, req_id):
        """Cria a bolha de preview do streaming (rodar na thread da UI)."""
        ...
        if self.eve_avatar:
            ...
            ctk.CTkLabel(container, text="⚡", font=ctk.CTkFont(size=28),
                         text_color=self.colors["accent"]).pack(side="left", padx=5)
        ...
        name_row = ctk.CTkFrame(bubble, fg_color="transparent")
        name_row.pack(anchor="w", padx=20, pady=(12, 0))
        ctk.CTkLabel(name_row, text="EVE", font=ctk.CTkFont(size=10, weight="bold"),
                     text_color=self.colors["accent"]).pack(side="left")
        ctk.CTkLabel(name_row, text=" • digitando…", font=ctk.CTkFont(size=9),
        ...
        label.pack(padx=20, pady=(6, 14))

        self._stream_widgets = {"req_id": req_id, "container": container, "label": label}

    def _update_stream_bubble(self, req_id, text):
        """Atualiza o texto da bolha de streaming (rodar na thread da UI)."""
        w = getattr(self, "_stream_widgets", None)
        if not w or w["req_id"] != req_id or self.active_request != req_id:
            return
        try:
    ...
    def _discard_stream_bubble(self):
        """Remove a bolha de preview (a resposta final entra em bolha própria)."""
        w = getattr(self, "_stream_widgets", None)
        if not w:
            return
        try:
    ...
    def send_message(self):
        ...
        # Identifica esta requisição (respostas de pedidos antigos são descartadas)
        self.request_counter += 1
        req_id = self.request_counter
        self.active_request = req_id

        # Processa em thread
        threading.Thread(target=self.process_message, args=(msg, files_to_send, req_id), daemon=True).start()
    ...
    def process_message(self, msg, files, req_id):
        """Processa mensagem (thread separada) - com timeout e fallback"""
        # PROTEÇÃO TOTAL: Captura QUALQUER erro e mostra resposta sem crashar
        try:
            self._process_message_internal(msg, files, req_id)
            ...
            if self.active_request != req_id:
                return  # Pedido já expirou (timeout) — não mexe na UI

            # Mostra erro para o usuário sem crashar
            self.after(0, self._discard_stream_bubble)
    ...
    def _process_message_internal(self, msg, files, req_id):
        """Processamento interno de mensagem"""

        # Timeout: libera a UI e invalida o pedido. A geração continua em
        # background, mas a resposta atrasada será descartada (req_id antigo).
        def timeout_handler():
            if self.active_request != req_id:
                return  # Resposta já chegou ou outro pedido assumiu
            print("[DEBUG] ⏰ TIMEOUT atingido - liberando UI, resposta será descartada")
            self.active_request = None
            self.after(0, self._discard_stream_bubble)
            self.after(0, lambda: self.create_bubble(
        ...
        # Streaming: mostra o texto chegando enquanto o modelo local gera.
        # (Groq responde de uma vez, então não emite chunks — sem preview.)
        stream_state = {"text": "", "started": False}
        ...
        def on_stream_chunk(chunk):
            ...
            if not stream_state["started"]:
                ...
                self.after(0, lambda: self._start_stream_bubble(req_id))
            texto_parcial = stream_state["text"]
            self.after(0, lambda t=texto_parcial: self._update_stream_bubble(req_id, t))
        ...
        try:
            ...
            # Timer de timeout (120s: código no Groq pode levar mais de 60s)
            timeout_timer = threading.Timer(120.0, timeout_handler)
            timeout_timer.daemon = True
            timeout_timer.start()

            # Gera a resposta através do motor principal da EVE
            # eve_lock serializa o acesso: o motor não é thread-safe e um
            # pedido que sofreu timeout pode ainda estar rodando
            ...
            try:
                with self.eve_lock:
                    response = self.eve.generate_response(
                        ...
                        files=files,
                        max_tokens=2500 if use_groq else 1500,
                        stream_callback=None if files else on_stream_chunk,
                    )
                timeout_timer.cancel()  # Cancela timer se completou

                # Pedido expirou (timeout já mostrou mensagem)? Descarta.
                if self.active_request != req_id:
                    print("[DEBUG] ⚠️ Resposta descartada - pedido expirado por timeout")
                    return
                ...
                self.after(0, self._discard_stream_bubble)
                if self.active_request != req_id:
                    print("[DEBUG] ⚠️ Erro descartado - pedido expirado por timeout")
                    return
...
{chr(10).join(f'  • {t}' for t in topics[:5])}"""
    ...
    def save_all_chats(self):
        ...
        try:
            with open("data/eve_chats.json", "w", encoding="utf-8") as f:
    ...
    def load_chats(self):
        """Carrega chats salvos"""
        try:
            if os.path.exists("data/eve_chats.json"):
                with open("data/eve_chats.json", "r", encoding="utf-8") as f:
                    return json.load(f)
        except Exception:
            pass
        return {}
